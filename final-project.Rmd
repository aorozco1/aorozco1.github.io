...
Would like to examine what is the best predictor for the a game's sales being successful. We will define "successful" as being double the mean number of sales (which is 536,360 total units sold globally). To start off, we will look at a logistic regression for critic sales.
```{r}
...
is_top_publisher <- videoGameData %>%
 ...
 filter(!is.na(Critic_Score)) %>%
 filter(!is.na(User_Score)) %>%
  dplyr::select(Name,Publisher,Global_Sales,isTopPublisher,isSuccessful)
...
```{r}
fit_is_top <- glm(isSuccessful~ isTopPublisher,data=is_top_publisher,family=binomial) 
fit_is_top %>%
  broom::tidy()
...
```{r}
lda_fit <- MASS::lda(isSuccessful ~ isTopPublisher, data=is_top_publisher)
...
```{r}
mean_score <- mean(greater_than_mean$Critic_Score)
mean_game_data  <- greater_than_mean%>%
  mutate(higher_than_avg = ifelse(mean_score > greater_than_mean$Critic_Score,FALSE,TRUE))

```


```{r}
lda_fit <- MASS::lda(isSuccessful ~ Global_Sales, data=mean_game_data)
lda_pred <- predict(lda_fit, data=mean_game_data)
print(table(predicted=lda_pred$class, observed=mean_game_data$isSuccessful))
#error rate
mean(mean_game_data$isSuccessful != lda_pred$class) * 100
# dummy error rate
mean(mean_game_data$isSuccessful!= FALSE) * 100
